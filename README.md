...
| Админ-панель | 5 вкладок: редактор статей + SEO-чеклист, синдикация, ТЗ, реестр тем, семантическая карта (полосы объёма, дерево бурения, подтверждено рекламой) | `hub/admin/web/` |
| Backend админки | Yandex Cloud Function, CRUD поверх Object Storage | `hub/admin/function/` |
| Скрипты публикации и семантики | синк с бакетом, снятие частотности по Wordstat, рекурсивное бурение семантики, свод рекламных данных | `hub/admin/scripts/` |
...
> «Стаб» — фактура собрана только с публичного лендинга продукта, без полного брифа от Дизрапта; не использовать для рекламных текстов без проверки, для контентных статей — достаточно как отправная точка (см. каждый `PRODUCT.md`).
...
Сборка семантики — намеренно не одноразовый запрос по теме, а трёхслойная
проверка объёма, где живая реклама и рекурсивный разбор дерева важнее
готовой формулы. Полная методология — `hub/HUB.md`, разделы «1. Бэклог тем»
и «Семантическая карта: полосы объёма спроса».

**Слой 1 — дерево вглубь, не один срез.** Один запрос к Wordstat отдаёт
только топ-10 фраз, похожих ровно на заданную формулировку — он физически
не показывает, что скрывается под каждой из них. `hub/admin/scripts/
drill-semantic-tree.py` рекурсивно бурит перспективные ветки вторым,
третьим запросом (топ-5 детей на узел, дальше бурится то, что ≥ 3000, в
дерево не попадает ничего < 500), пока ветка остаётся продуктивной —
результат в `hub/admin/web/data/semantic-tree.json`, виден во вкладке
админки «Семантическая карта» → «Дерево семантики» с двумя независимыми
статусами на узел (бурение: пробурено/бурить дальше; содержание: есть
статья/потенциал статьи/нет потенциала/узко).

**Слой 2 — подтверждено рекламой, когда данные есть.** Там, где у продукта
уже крутится платный трафик, `hub/admin/scripts/build-semantic-map.py`
сверяет реальные конверсии Яндекс.Директа с уже написанными темами и явно
показывает разрыв — например, самая конвертящая рекламная группа Мультитула
может не иметь вообще ни одной SEO-темы под себя. Этот сигнал сильнее
любой оценки по Wordstat, потому что это не потенциал, а состоявшийся спрос.

**Слой 3 — целевая полоса объёма, когда рекламных данных нет.**
`hub/admin/scripts/apply-topic-frequency.py` прогоняет короткие головные
фразы (`hub/admin/scripts/topic-phrases.tsv`) через Wordstat и пишет
широкую частотность прямо в `hub/topics/*-backlog.md`, идемпотентно.
Целевой ориентир — полоса 5 000–20 000 (перенесена из реального
семантического ядра агентства giga.chat, не произвольное число): не
сверхширокая голова (домену без истории индексации её не отдадут) и не
сверхузкий хвост (трафика физически нет).

**Жёсткий порог видимости.** Тема без измеренной частотности **или** с
частотностью ниже 500 не показывается в «Реестре тем» вообще — отсутствие
данных не освобождает от порога, это подтверждённое отсутствие спроса, а
не «ещё не проверили» (см. `hub/HUB.md`, «Жёсткие правила»). Приоритет
внутри прошедших порог тем — прозрачная формула `P/traf-оценка × вес
интента × вес продуктового соответствия`, все компоненты видны отдельными
колонками в дашборде, финальное решение всегда за человеком.
...
└── hub/
    ...
    └── admin/                         — АДМИН-ПАНЕЛЬ И BACKEND
        ...
        ├── web/                       — статический фронтенд админки (vanilla JS), 5 вкладок:
        │   │                            Статьи для хаба · Для блогов · ТЗ · Реестр тем · Семантическая карта
        │   ├── index.html · app.js · styles/
        │   └── data/                  — снапшоты для дашбордов (генерируются скриптами ниже)
        │       ├── topics.json                    — реестр тем
        │       ├── topic-frequency-details.json   — разбивка по похожим/смежным запросам Wordstat
        │       ├── semantic-map.json              — полосы объёма + зоны роста + подтверждено рекламой
        │       └── semantic-tree.json              — дерево рекурсивного бурения Wordstat
        └── scripts/                   — локальные инструменты обслуживания
            ├── publish.mjs               — публикация локального файла статьи в бакет
            ├── publish-brief.mjs         — публикация ТЗ в бакет (_briefs/<product>/<topicId>.md)
            ├── publish-syndication.mjs   — публикация пакета синдикации (_syndication/<platform>/<topicId>.md)
            ├── sync-topics.mjs           — бэклоги → topics.json
            ├── apply-topic-frequency.py  — Wordstat (один срез) → бэклоги + topic-frequency-details.json
            ├── drill-semantic-tree.py    — Wordstat (рекурсивное бурение) → semantic-tree.json
            ├── build-semantic-map.py     — сводит topics/details/рекламный отчёт → semantic-map.json
...
- **Двухступенчатый гейт: тема → ТЗ → черновик.** Гейт 1 («идея → утверждено») проходит либо правкой `hub/topics/*-backlog.md` в git, либо живым одобрением прямо из админ-дашборда (не требует правки файла) — приоритет: написанная статья > живое одобрение > статус из git-файла. Только после Гейта 1 материализуется ТЗ файлом (`_briefs/<product>/<topicId>.md`, вкладка админки «ТЗ»); Гейт 2 («ТЗ утверждено») — отдельное решение человека до того, как начинается черновик. Ни агент, ни автоматизация не проходят ни один из двух гейтов сами.
...
- **Синдикация** (Dzen, VC, Habr) — вкладка админки «Для блогов», только после индексации оригинала, со своим заголовком-хуком и открывающим абзацем под площадку, никогда дословным копипастом текста статьи.
- **UGC** — критерии модерации и меню стимулов описаны в методологии; форма приёма статей от пользователей — зона Дизрапта, не контентного движка.
...
- Рекламные креативы, баннер-кит, медиапланы, отчёты по перформансу, брендбуки продуктов, данные кампаний, бюджетные xlsx — зона отдельных рабочих процессов, не имеет отношения к написанию и публикации статей. Исключение — `build-semantic-map.py` читает отчёт Яндекс.Директа (`../reports/<DD.MM>/Disrupt-<DD-MM>.xlsx` в основном рабочем дереве, вне этого репозитория) как один из сигналов приоритизации тем; при отсутствии файла скрипт молча пропускает этот слой, остальная сборка карты не ломается.
